...
public class App extends Application {
    ...
    public void start(Stage stage) {
        ...
        VBox vBoxText = new VBox(10, textField, labelResult);
         ...
         titledPaneFont.setVisible(false);
         ...
         titledPaneCharColor.setVisible(false);
         ...
         redSlider.valueProperty().addListener(
                 (observable, oldValue, newValue) -> updateColor(labelResult, redSlider, greenSlider, blueSlider));
         ...
         titledPaneCamel.setVisible(false);
        ...
        String[] array = { "Couleur de fond", "Couleur des caractères", "Casse" };

        for (String option : array) {
            CheckBox checkBox = new CheckBox(option);

            // gérer un évènement de clic sur la CheckBox
            checkBox.setOnAction(event -> {
                CheckBox target = (CheckBox) event.getTarget();
                String targetString = target.getText();

                if (target.isSelected() == true){
                    if (targetString.equals(array[0])){
                        titledPaneFont.setVisible(true);

                    } else if (targetString.equals(array[1])){
                        titledPaneCharColor.setVisible(true);

                    } else {
                        titledPaneCamel.setVisible(true);
                    }
                }
    ...
    private void updateFont(Label label, String color) {
        ...
        // déclaration d'un objet de la classe BackgroundFill permettant de changer la
        // couleur
        // d'un label
    ...
    private void updateColor(Label label, Slider redSlider, Slider greenSlider, Slider blueSlider) {
        ...
        // Version avec utilisation de Java pour le changement de la couleur de fond du
        // Label
        // dans ce cas il suffit de changer le style CSS du label
        label.setStyle("-fx-text-fill: " + hexaColor + ";");

        // ------- Version en modifiant le CSS
        // Cas plus complexe car il faut faire attention de ne changer QUE la valeur de
        // la couleur du texte
        ...
        // } else {
        // // ce n'est pas un style de couleur de texte (probablement un style de
        // backgroud)
